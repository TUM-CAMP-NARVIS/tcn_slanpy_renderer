...
from slangpy_renderer.renderers import ColoredMeshRenderer
...
def main(argv: list[str] | None = None) -> None:
    ...
    # --- Load axis mesh ---
    origin = ColoredMesh.create_axis3d(device)
    origin.pose = np.eye(4, dtype=np.float32)

    # --- Create renderer and depth buffer ---
    renderer = ColoredMeshRenderer(device, output_format)
    origin.renderer = renderer
